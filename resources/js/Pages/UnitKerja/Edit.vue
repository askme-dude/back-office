...
const form = useForm({
    nama: props.dataUnit.nama,
    jenis_unit_kerja_id: props.dataUnit.jenis_unit_kerja_id,
    singkatan: props.dataUnit.singkatan,
    keterangan: props.dataUnit.keterangan,
})
const simpanUnit = ()=>{
    form.put(route('unit-kerja.update',props.dataUnit),{
        preserveScroll:true,
        preserveState:true,
        replace:true,
        onSuccess:(response)=>{
            Swal.fire({
                title: 'Tersimpan!',
                text: 'Data Unit Kerja Berhasil Diubah!',
                icon: 'success',
...
<template>
    <div class="text-sm breadcrumbs">
        <ul>
            ...
            <li><span class="text-info">Edit Unit Kerja</span></li>
        </ul>
    </div>

    <MainCard>
        <div class="w-full p-6 m-auto lg:max-w-xl">
            <h2 class="text-2xl font-semibold text-center text-gray-700">Edit Unit Kerja</h2>
            <form class="space-y-4" @submit.prevent="simpanUnit">
